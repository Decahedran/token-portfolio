...
const isVercel = !!process.env.VERCEL; // true in Vercel prod/preview

// ---------- Memory store (safe fallback for production without KV) ----------
...
async function ensureFile(): Promise<boolean> {
  try {
    ...
    // Read-only FS (e.g., Vercel prod) — signal not usable
    return false;
  }
}
...
async function writeAll(obj: Record<string, unknown>): Promise<void> {
  const ok = await ensureFile();
  if (!ok) return; // silently ignore if FS is not writable
...
// 1) If KV env is present → use KV (persistent)
// 2) Else if running on Vercel → use in-memory fallback (no 500s)
